...
PROMPT_MODEL = """
あなたは自動車モデル名の正規化を行う変換器です。出力は厳密に JSON のみ。
...
2) 記号・英数字・スペース（Model 3, DM-i, Pro, MAX, Plus 等）は保持。
3) ブランドが先頭にある場合（例: 本田CR-V）はブランド片を削除し、**モデルのみ**にする。

【優先順位ルール（辞書に頼らず一般化）】
A) **グローバルに通用する英名**が明確な場合は英名を採用。
   - BYD 海洋シリーズは英名固定：海獅→Sea Lion, 海豹→Seal, 海豚→Dolphin, 海鷗→Seagull
   - 輸出名が定着している中国モデルは英名優先：
     朗逸→Lavida, 速腾→Sagitar, 迈腾→Magotan, 探岳→Tayron, 途岳→Tharu, 途観L→Tiguan L,
     锋兰达→Frontlander, 卡罗拉锐放→Corolla Cross, 缤越→Coolray, 博越L→Atlas L, 瑞虎8→Tiggo 8, 艾瑞泽8→Arrizo 8 など
   - 小型派生：元UP→Dolphin Mini, 元PLUS→Atto 3
   - モデル番号/英字はそのまま（SU7, CS75 PLUS, A6L など）
B) **日本で長年使われる既定カタカナ車名**はカタカナを優先。
   例: シルフィー（轩逸）, アコード（雅阁）, カムリ（凯美瑞）, カローラ など
C) **グローバル名が不明で中国語固有名**の場合は、**簡体字→日本語の新字体に変換**し、
   かつ**ピンインをシリーズ名直後に括弧で併記**する。
   - 例: 星愿→星願（Xingyuan）, 逸動→逸動（Yidong）
   - 複合語の場合は**シリーズ本体の直後**にピンインを付与し、接尾の記号/サフィックスは保持。
     例: 宏光MINIEV→**宏光（Hongguang）MINIEV**
D) グレード/動力派生（Pro/PLUS/MAX/EV/PHEV/DM-i 等）はそのまま保持。
E) 誤カナ化・当て字は避け、安定した国際表記に揃える。
