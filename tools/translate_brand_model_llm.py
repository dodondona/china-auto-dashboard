...
# -*- coding: utf-8 -*-
"""
translate_brand_model_llm.py  （Wikipediaのみ版）
...
def get_langlinks_by_title(zh_title: str) -> Dict[str, Optional[str]]:
    """
    zhタイトルから言語間リンク(en/ja)を取得。リダイレクトは自動追従。
    """
    data = wiki_query(
        {
            "action": "query",
...
def search_zh_title(kw: str) -> Optional[str]:
    """
    近似検索→通常検索の順に zh タイトル候補を探す。
    """
    data = wiki_query(
        {"action": "query", "list": "search", "srsearch": kw, "srwhat": "nearmatch", "srlimit": 1}
    )
    hits = data.get("query", {}).get("search", [])
    if hits:
        return hits[0].get("title")

    data2 = wiki_query({"action": "query", "list": "search", "srsearch": kw, "srlimit": 1})
...
def resolve_name_via_wikipedia(zh_name: str, sleep_sec: float = 0.1) -> Dict[str, str]:
    """
    1) zh名がASCIIならそのまま返す
    2) zhタイトル → 言語間リンクで ja/en を取得（なければ検索→再取得）
    3) 優先順位: ja > en > zh（いずれも無ければ元のzh）
    """
...
def main():
    ...
    # ✅ 後方互換のためのエイリアスを追加（--brand/--model/--brand-ja/--model-ja）
