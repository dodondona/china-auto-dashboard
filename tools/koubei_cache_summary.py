...
# 重要：ハング防止
PAGE_NAV_TIMEOUT_MS = int(os.environ.get("PAGE_NAV_TIMEOUT_MS", "15000"))  # 15s/ページ
PAGE_RETRIES = int(os.environ.get("PAGE_RETRIES", "2"))                    # 失敗時の再試行
WALL_TIMEOUT_SEC = int(os.environ.get("WALL_TIMEOUT_SEC", "480"))          # 全体の壁時計リミット
...
def upsert_jsonl_by_key(path: Path, obj: Dict[str, Any], key: str) -> None:
    ...
    if idx is None: rows.append(obj)
    else: rows[idx] = obj
...
def parse_reviews(html: str) -> List[Dict[str, Any]]:
    ...
    for a in soup.select("a[href]"):
        ...
        items.append({"review_id": rid, "title": title, "content": content, "pub_date": pub, "score": score})
    seen=set(); uniq=[]
    for it in items:
...
def compose_story(title: str, fields: Dict[str,Any], score: str, text: str) -> str:
    parts = []
...
def polish_with_llm(text_ja: str) -> str:
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            temperature=0.2,
            max_tokens=450,
            timeout=20,  # SDKが対応している環境では有効
        )
        return resp.choices[0].message.content.strip()
    except Exception as e:
        log(f"LLM polish failed: {e}")
        return text_ja
...
def run(series_id: str) -> None:
    ...
    with sync_playwright() as p:
        ...
        context = browser.new_context(
            user_agent=os.environ.get("UA_OVERRIDE", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0 Safari/537.36"),
            viewport={"width":1280, "height":2000}
        )
        pageobj = context.new_page()
        pageobj.set_default_navigation_timeout(PAGE_NAV_TIMEOUT_MS)

        def goto_with_retry(url: str) -> bool:
            for i in range(PAGE_RETRIES+1):
                try:
                    log(f"goto({url}) try={i+1}")
                    pageobj.goto(url, wait_until="domcontentloaded")
                    pageobj.wait_for_timeout(1200)
                    return True
                except PWTimeout:
                    log(f"timeout: {url}")
            return False
        ...
        for page in range(1, MAX_PAGES+1):
            if time.time() - start > WALL_TIMEOUT_SEC:
                log("WALL TIMEOUT reached — abort.")
                break

            url = (f"https://k.autohome.com.cn/{series_id}#pvareaid=3454440"
                   if page == 1 else
                   f"https://k.autohome.com.cn/{series_id}/index_{page}.html?#listcontainer")

            ok = goto_with_retry(url)
            if not ok:
                log(f"skip page {page} due to repeated timeouts")
                continue

            html = pageobj.content()
            revs = parse_reviews(html)
            log(f"page={page} parsed {len(revs)} candidates")
            ...
            for r in revs:
                ...
                if is_already_summarized(rid, chash, SUMMARY_FMT_VER, OPENAI_MODEL if USE_LLM else "no-llm"):
                    seen_consec_known += 1
                    continue
                ...
                base = compose_story(r.get("title",""), fields, r.get("score",""), content)
            ...
            log(f"page={page} new={page_new_any} consec_known={seen_consec_known}")
            if seen_consec_known >= STOP_AFTER_CONSEC_KNOWN:
                log(f"hit STOP_AFTER_CONSEC_KNOWN={STOP_AFTER_CONSEC_KNOWN}, stop crawl.")
                break
    ...
    log(f"[series {series_id}] total_seen={total_seen} new_summaries={new_summaries} elapsed={int(time.time()-start)}s")
    print(f"[series {series_id}] DONE")
