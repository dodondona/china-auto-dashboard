...
def main():
    ...
    cn_snap_path = Path(CACHE_REPO_DIR)/str(SERIES_ID)/"cn.csv"
    ja_prev_path = Path(CACHE_REPO_DIR)/str(SERIES_ID)/"ja.csv"
    ...
    enable_reuse = (not FORCE_RETRANSLATE) and (prev_cn_df is not None) and (prev_ja_df is not None) and (df.shape==prev_cn_df.shape) and (list(df.columns)==list(prev_cn_df.columns))
    ...
    for i in range(len(df)):
        for j in range(4, len(df.columns)):
            ...
            coords.append((i,j))
    uniq_vals = uniq(uniq_vals)
    print(f"🌐 to_translate={len(uniq_vals)}")

    val_map = tr.translate_unique(uniq_vals) if uniq_vals else {}
    for (i,j) in coords:
        s=str(df.iat[i,j]).strip()
        if s: df.iat[i,j]=val_map.get(s,s)

    # 出力：セクション/項目は _ja のみ保持
    final_out = pd.concat([df[["セクション","項目"]].rename(columns={"セクション":"セクション_ja","項目":"項目_ja"}), df.iloc[:,4:]], axis=1)
    ...
    final_out.to_csv(ja_prev_path,index=False,encoding="utf-8-sig")

    print(f"✅ Saved: {DST_PRIMARY}")
    print(f"📦 Cache CN: {cn_snap_path}")
    print(f"📦 Cache JA: {ja_prev_path}")
