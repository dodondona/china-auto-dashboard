...
Outputs:
  - autohome_reviews_<ID>.batch.input.jsonl   (Batch API 入力ファイル)
  - autohome_reviews_<ID>.batch.submit.json   (batch_id, input_file_id 等のメタ情報)
  - 標準出力に batch_id を表示
"""
...
COMPLETION_WINDOW = os.environ.get("BATCH_COMPLETION_WINDOW", "24h")  # 24h のみ
...
def parse_reviews(html:str)->List[str]:
    ...
    # Autohome 口コミブロックを素朴に抽出（既存コード準拠の緩めパターン）
